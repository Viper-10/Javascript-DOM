...
const winningNumber = parseInt(Math.floor(Math.random()* (end-start) + start));
...
function checkGuess(e){
  ...
  if(guess === winningNumber){
    setMessageOutput('green', `You Won!!! ${winningNumber} is the correct guess`)
    gameOver(); 
    return;
  }  
    ...
    if(numberOfGuessesLeft === 0){
      setMessageOutput('red', `You Lost, ${winningNumber} is the correct number`);
      gameOver(); 
    }
    else{
      if(guess > winningNumber) {
        setMessageOutput('red', `${guess} is larger than the winning number, you have ${numberOfGuessesLeft} more guesses`) 
      }

      else{
        setMessageOutput('red', `${guess} is smaller than the winning number,you have ${numberOfGuessesLeft} more guesses`);
      }
    }
  }
}
...
function gameOver(){
  ...
  playAgainText.appendChild(document.createTextNode('Click Play again to play again'))
